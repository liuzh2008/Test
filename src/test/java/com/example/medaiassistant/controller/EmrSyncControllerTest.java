...
/**
 * EmrSyncController 外部集成测试
 * 任务4：验证EMR病历内容同步REST API控制器
 * 
 ...
 *   <li>CT-05: 批量导入成功返回200 ✅</li>
 *   <li>CT-06: 空列表返回400 ✅</li>
 *   <li>CT-07: null patientId返回400 ✅</li>
 *   <li>CT-08: 统计信息返回正确数据 ✅</li>
 *   <li>CT-09: 健康检查响应时间验证 ✅</li>
 * </ul>
 * 
 * <p>测试结果：Tests run: 9, Failures: 0, Errors: 0, Skipped: 0</p>
 * 
 * @author TDD
 * @version 1.0
 * @since 2026-01-11
 */
@DisplayName("EmrSyncController 外部集成测试")
class EmrSyncControllerTest {
    ...
    /**
     * 测试：健康检查端点应返回200状态码
     * 验收标准：GET /api/emr-sync/health 返回200，包含status=UP
     */
    @Test
    @DisplayName("CT-01: 健康检查应返回200")
    void testHealthCheck_ReturnsUp() throws Exception {
        // Given - 构建健康检查URL
        String url = buildApiUrl("emr-sync/health");
        ...
        assertEquals("EmrSyncService", root.get("service").asText(), "服务名称应为EmrSyncService");
     ...
     * 验收标准：POST /api/emr-sync/import 成功时返回200，success=true
     * 注意：此测试需要系统中存在有效的测试患者数据
     */
    @Test
    @DisplayName("CT-02: 单个导入成功应返回200")
    void testImport_Success_Returns200() throws Exception {
        // Given - 构建导入URL和请求体（使用测试患者ID）
        String url = buildApiUrl("emr-sync/import");
     ...
     * 验收标准：POST /api/emr-sync/import 失败时返回500，success=false
     */
    @Test
    @DisplayName("CT-03: 单个导入失败应返回500")
    void testImport_PatientNotFound_Returns500() throws Exception {
     ...
     * 验收标准：POST /api/emr-sync/import 空ID返回400
     */
    @Test
    @DisplayName("CT-04: 空patientId应返回400")
    void testImport_InvalidPatientId_Returns400() throws Exception {
    ...
    /**
     * 测试：批量导入应返回200状态码
     * 验收标准：POST /api/emr-sync/batch-import 返回200，包含统计信息
     */
    @Test
    @DisplayName("CT-05: 批量导入成功应返回200")
    void testBatchImport_Success_Returns200() throws Exception {
        // Given - 构建批量导入URL和请求体
        String url = buildApiUrl("emr-sync/batch-import");
     ...
     * 验收标准：POST /api/emr-sync/batch-import 空列表返回400
     */
    @Test
    @DisplayName("CT-06: 空列表批量导入应返回400")
    void testBatchImport_EmptyList_Returns400() throws Exception {
     ...
     * 验收标准：POST /api/emr-sync/import 无patientId字段返回400
     */
    @Test
    @DisplayName("CT-07: null patientId应返回400")
    void testImportWithNullPatientId_Returns400() throws Exception {
    ...
    /**
     * 测试：统计信息端点应返回正确的统计数据
     * 验收标准：GET /api/emr-sync/stats?patientId=xxx 返回200，包含统计信息
     */
    @Test
    @DisplayName("CT-08: 统计信息应返回正确数据")
    void testStats_ReturnsCorrectCounts() throws Exception {
        // Given - 构建统计信息URL
        String url = buildApiUrl("emr-sync/stats?patientId=990500000178405-1");
        ...
        // Then - 验证响应
        assertEquals(200, response.statusCode(), "统计信息应返回200状态码");
        
        JsonNode root = objectMapper.readTree(response.body());
        assertNotNull(root.get("patientId"), "响应应包含patientId字段");
        assertNotNull(root.get("localEmrCount"), "响应应包含localEmrCount字段");
        assertTrue(root.get("localEmrCount").asLong() >= 0, "localEmrCount应>=0");
    }
    ...
    /**
     * 测试：健康检查响应时间应在合理范围内
     * 验收标准：GET /api/emr-sync/health 响应时间应小于1秒
     */
    @Test
    @DisplayName("CT-09: 健康检查响应时间应小于1秒")
    void testHealthCheckResponseTime() throws Exception {
        ...
        System.out.println("CT-09: 健康检查响应时间: " + responseTime + "ms");
    }
    ...
    /**
     * 构建业务API端点URL
     * @param path 端点路径（如 emr-sync/health）
     * @return 完整URL（如 http://localhost:8081/api/emr-sync/health）
     */
