...
/**
 * OrderSyncController 外部集成测试
 * 
 * <p>使用 SystemAvailabilityChecker API 进行系统可用性检查</p>
 * <p>直接对运行中的后端服务发起HTTP请求，不加载Spring上下文</p>
 * 
 * <p><strong>测试端点</strong>：</p>
 * <ul>
 *   <li>GET /api/order-sync/health - 健康检查</li>
 *   <li>POST /api/order-sync/import - 单个患者医嘱导入</li>
 *   <li>POST /api/order-sync/batch-import - 批量患者医嘱导入</li>
 * </ul>
 * 
 * @author TDD
 * @version 1.0
 * @since 2026-01-10
 */
@DisplayName("OrderSyncController 外部集成测试")
class OrderSyncControllerTest {
    ...
    /**
     * 测试：健康检查端点应返回200状态码
     * 验收标准：GET /api/order-sync/health 返回200，包含status=UP
     */
    @Test
    @DisplayName("CT-01: 健康检查应返回200")
    void testHealthCheck() throws Exception {
        // Given - 构建健康检查URL
        String url = buildApiUrl("order-sync/health");
     ...
     * 验收标准：POST /api/order-sync/import 成功时返回200，success=true
    ...
    void testImportSuccess() throws Exception {
        ...
        String url = buildApiUrl("order-sync/import");
     ...
     * 验收标准：POST /api/order-sync/import 失败时返回500，success=false
     */
    @Test
     ...
     * 验收标准：POST /api/order-sync/import 空ID返回400
     */
    @Test
    ...
    // ==================== CT-05: null patientId测试 ====================

    /**
     * 测试：null patientId应返回400状态码
     * 验收标准：POST /api/order-sync/import 无patientId字段返回400
     */
    @Test
    @DisplayName("CT-05: null patientId应返回400")
    ...
    /**
     * 测试：批量导入应返回200状态码
     * 验收标准：POST /api/order-sync/batch-import 返回200，包含统计信息
     */
    @Test
    @DisplayName("CT-06: 批量导入成功应返回200")
    void testBatchImportSuccess() throws Exception {
        // Given - 构建批量导入URL和请求体
        String url = buildApiUrl("order-sync/batch-import");
     ...
     * 验收标准：POST /api/order-sync/batch-import 空列表返回400
     */
    @Test
    @DisplayName("CT-07: 空列表批量导入应返回400")
    ...
    /**
     * 测试：批量导入部分失败应正确统计
     * 验收标准：POST /api/order-sync/batch-import 部分失败时failCount正确
     */
    @Test
    @DisplayName("CT-08: 批量导入部分失败应正确统计")
    void testBatchImportPartialFailure() throws Exception {
        // Given - 构建批量导入URL和请求体（包含有效和无效的患者ID）
        String url = buildApiUrl("order-sync/batch-import");
        String requestBody = "{\"patients\":[" +
                "{\"patientId\":\"990500000178405-1\"}," +
                "{\"patientId\":\"invalid-patient-id-not-exist\"}" +
                "]}";
        ...
        JsonNode root = objectMapper.readTree(response.body());
        assertNotNull(root.get("success"), "响应应包含success字段");
        assertEquals(2, root.get("totalPatients").asInt(), "totalPatients应为2");
        assertTrue(root.get("failCount").asInt() >= 1, "failCount应>=1（至少有1个无效ID）");
    }
    ...
    /**
     * 测试：健康检查响应时间应在合理范围内
     * 验收标准：GET /api/order-sync/health 响应时间应小于1秒
     */
    @Test
    ...
    /**
     * 构建业务API端点URL
     * @param path 端点路径（如 order-sync/health）
     * @return 完整URL（如 http://localhost:8081/api/order-sync/health）
     */
